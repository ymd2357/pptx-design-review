...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # --- FIX-008: slot-aware card_grid medians (text widens, image preserved) ---
        synthetic_row_containers = [
            # Card A (consistent reference): icon + short title + body
            {
                "shape_id": 100,
                "kind": "shape",
                "bbox_pt": [0.0, 0.0, 300.0, 200.0],
                "children": [
                    {
                        "shape_id": 101,
                        "kind": "image",
                        "source_order_index": 1,
                        "bbox_pt": [20.0, 20.0, 32.0, 32.0],
                    },
                    {
                        "shape_id": 102,
                        "kind": "text",
                        "source_order_index": 2,
                        "bbox_pt": [60.0, 20.0, 200.0, 40.0],
                    },
                    {
                        "shape_id": 103,
                        "kind": "text",
                        "source_order_index": 3,
                        "bbox_pt": [20.0, 80.0, 260.0, 90.0],
                    },
                ],
            },
            # Card B (consistent reference): same template, same widths
            {
                "shape_id": 200,
                "kind": "shape",
                "bbox_pt": [320.0, 0.0, 300.0, 200.0],
                "children": [
                    {
                        "shape_id": 201,
                        "kind": "image",
                        "source_order_index": 4,
                        "bbox_pt": [340.0, 20.0, 32.0, 32.0],
                    },
                    {
                        "shape_id": 202,
                        "kind": "text",
                        "source_order_index": 5,
                        "bbox_pt": [380.0, 20.0, 200.0, 40.0],
                    },
                    {
                        "shape_id": 203,
                        "kind": "text",
                        "source_order_index": 6,
                        "bbox_pt": [340.0, 80.0, 260.0, 90.0],
                    },
                ],
            },
            # Card C (inconsistent): title is too narrow → should widen to 200
            {
                "shape_id": 300,
                "kind": "shape",
                "bbox_pt": [640.0, 0.0, 300.0, 200.0],
                "children": [
                    {
                        "shape_id": 301,
                        "kind": "image",
                        "source_order_index": 7,
                        "bbox_pt": [660.0, 20.0, 32.0, 32.0],
                    },
                    {
                        "shape_id": 302,
                        "kind": "text",
                        "source_order_index": 8,
                        "bbox_pt": [700.0, 20.0, 120.0, 40.0],
                    },
                    {
                        "shape_id": 303,
                        "kind": "text",
                        "source_order_index": 9,
                        "bbox_pt": [660.0, 80.0, 260.0, 90.0],
                    },
                ],
            },
        ]
        slot_medians = pptx_fix._card_grid_slot_medians(synthetic_row_containers)
        if sorted(slot_medians.keys()) != ["0", "1", "2"]:
            failures.append(
                f"slot_medians expected keys 0/1/2 (sorted by top,left); got {list(slot_medians.keys())}"
            )
        else:
            slot0 = slot_medians["0"]
            if slot0.get("kind") != "image" or slot0.get("sample_count") != 3:
                failures.append(
                    f"slot[0] should be image x3 (sorted by top first → icon at top y=20 with rel_left=20); got {slot0}"
                )
            slot1 = slot_medians["1"]
            if slot1.get("kind") != "text" or abs(slot1.get("width", 0) - 200.0) > 0.01:
                failures.append(
                    f"slot[1] expected median text width 200 (cards A/B=200, C=120 → median=200); got {slot1}"
                )
            slot2 = slot_medians["2"]
            if slot2.get("kind") != "text" or abs(slot2.get("width", 0) - 260.0) > 0.01:
                failures.append(
                    f"slot[2] expected text width 260; got {slot2}"
                )
