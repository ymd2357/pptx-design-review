...
| POLICY-001 | partial | P1 | lint→fix の policy を YAML 正本テーブルに統一する | **段階1 (完了)**: `doc/slide-guideline-v1.yml` に `rules.lint.fix_policy` セクションを新設し、34 check 分の `{apply_mode: auto/judgement/manual, fix_rule: <rule>/null}` を宣言的に列挙。`apply_mode=auto` (5: text_autofit / font_size / line_height / alignment / badge_alignment)、`judgement` (23: contrast / bbox_fit / overlap / spacing / image_aspect / image_crop / reading_order / animation / text_wrap / heading_hierarchy / inner_padding / card_grid / text_vertical_balance / font_family / text_color / fill_color / geometry / object_gap_too_small 含む)、`manual` (6: image_upscale_ratio / alt_text_required / color_only_meaning / missing_required_element / decorative_isolated_lines / slide_size)。`skills/pptx-design-reviewer/scripts/test_fix_policy.py` で YAML 正本と `pptx_fix.CHECK_TO_RULE` の sync を assert。**段階2 (未着手)**: `pptx_lint.py:_fixability_for_json` と `pptx_fix.py` の if/elif 分岐を YAML 参照に refactor。**段階3 (未着手)**: 「apply_mode=judgement は judgement 不在で apply しない」厳格化 (= 現状の「candidate あれば即 apply」運用が変わるため仕様判断が要る)。 |
