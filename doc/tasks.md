...
| FIX-008 | done | P1 | `card_grid_consistency` の auto-fix を「保守的平行移動のみ」から **child の role / 関係性** を見る賢い修正に進化させる | `pptx_fix._card_grid_slot_medians()` を新設し、行内 card の children を **`(top, left)` ソート後の位置インデックス (= slot)** で横断的に median 集計するアプローチに切り替えた (`source_order_index` はスライド単位でユニークだったため不可)。slot 毎に `kind` (image/text/shape) と `width / height / rel_left / rel_top` の median を持ち、apply 時は kind 別に分岐: text なら slot median の width に揃え (= テンプレ意図のタイトル幅一致)、image / shape は元サイズ完全保持で位置のみ slot median offset へ移動。kind が card 間で混在する slot は保守的フォールバック (= v3 互換の平行移動)。<br>**lint 拡張**: `pptx_lint` の `row_containers` 要素にも `children` と `padding_pt` を埋め込み、行全体の slot 集計データを `pptx_fix` 側で扱えるようにした。<br>**REV-019 deck 再走結果**: image の cx/cy 全件不変 (v1 の回帰なし)、slide 7 でタイトル幅が `115.2pt → 168pt` (median) へ拡張される等 **可視な揃え効果**、container 単位 manual_required の数は v3 と同じ 4 件、post-fix lint は `card_grid_consistency 4 → 3` (= 1 行が完全消化)。<br>**追加観点 (FIX-012)**: より細かい role 推定 (`leading_icon` / `title` / `body` / `decorative` で title baseline / body left-padding を card 間で揃える等) は本 PR の slot ベースに上乗せして検出 lint を追加する余地あり。 |
| FIX-012 | todo | P2 | card_grid の role 推定 (title / body 区別) と role 別 lint 検知を追加 | FIX-008 で「slot index = card 内 (top, left) ソート順」までは入った。次は slot 毎に `role` (`leading_icon` / `title` / `body` / `decorative`) を高さ・幅・shape kind から推定し、(a) title baseline が card 間で揃っているか、(b) body の左 padding が card 間で揃っているか、を `card_grid_consistency` の triggered_rules に追加する。pptx_fix 側で role を知っていれば slot median ではなく role-specific transform を選べる (例: body は inner_width に揃え、title は左揃え + baseline 一致)。 |
