...
| DS-003 | partial | P1 | design-system 自体のレビューを追加する | `design_system_review.py` を新設し、3 check を実装: (1) `palette_family_tier_gap` (brand.primary/black/secondary の 10 tier 欠落)、(2) `repair_candidate_family_overlap` (同 hex が複数 contrast repair family に登場 = 混線)、(3) `repair_candidate_palette_drift` (repair_candidates の token が palette に存在しない or hex 不一致)。各 finding は evidence schema (check_id / evidence / fixability=manual_required / manual_required_reason 等) を満たす。現状の YAML against 実行で 1 件検出 (`data_series.series_6` が palette の flat list 構造のため token path として解決できない)。`test_design_system_review.py` でベースライン件数 (per check_id) を lock。**残り**: (a) 色用途 (`rules.color.allowlist.*` で各 palette 色が少なくとも 1 つの allowed_use にぶら下がっているか) の検出、(b) contrast pair の網羅性 (allowed × allowed の組み合わせのうち text_on_background / prohibited のどちらにも書かれていない gap) の検出、(c) data_series 構造の token path 化 (= 1 件残っている drift の根治)。 |
