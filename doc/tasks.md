...
| REV-033 | review | P1 | FIX-012 (role 推定 + 上揃え + badge 下揃え) の視覚レビュー | rev-017 deck を input、FIX-012 適用後を after として PNG capture。compare-meta が出した changed_slides = [2, 13, 20] (slide 20 は AE=1611 = render ノイズ、XML 不変)。観点別の意図: slide 2 = icon 4 つ・本文 4 つを y で揃える (= card 1/2 icon を 1 行目 center に上げる + card 3/4 body を下げる)、slide 13 = badge 2 つの top を揃える (= card 1 の badge を下げる)、slide 7 = no-op。自己評価で改善を確認済。SPA URL は `/compare/?deck=260329-seminar-curriculum-proposal&rev=033`。 |
