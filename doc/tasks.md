...
| ID | 状態 | 優先度 | タスク | 完了条件 |
| ---- | ------ | -------- | -------- | ---------- |
| WEB-001 | todo | P1 | レビュー UI を GitHub Pages の `review-pages` 専用ブランチで外部公開する | (1) `ymd2357/pptx-design-review` を public で新規作成し master を push、(2) repo Variables に `VITE_GITHUB_CLIENT_ID = Ov23liGOmUnBwrR9OVPO` を登録、(3) `deploy-review-web` ワークフローが `review-pages` ブランチに orphan push、(4) Settings → Pages の Source を `review-pages` ブランチ (`/`) に設定、(5) スマホから `https://ymd2357.github.io/pptx-design-review/` を開いて GitHub Device Flow でログイン → REV-017 の visual review にアクセスできる。REV-017 の判定はこの UI 経由で行う前提 |
