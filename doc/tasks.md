...
| FONT-002 | todo | P0 | lint/fix pipeline に FONT-001 正規化を必須前処理として組み込む | `pptx_lint.py` / `pptx_fix.py` の CLI で `--auto-normalize-fonts` (default ON) を追加、または `pptx_review_orchestrator.py` 等の上流で `pptx_normalize_fonts` を呼ぶ wrapper を統一。normalize されていない pptx を直接 lint/fix した場合に text width 計算が viewer の描画と乖離する (FONT-001 で確認済) ため、正規化を経由しないと estimator (HarfBuzz advance - rsb) と viewer 描画が一致しない。完了条件: 既存テストが normalize 経由でも pass、mcp-cource 等本番 deck を normalize 適用 + 再 fix → snapshot 更新 (text 切れが治る、box 位置の誤判定が消える) |
| FONT-003 | todo | P1 | text width estimator に fontTools / uharfbuzz を依存追加し font metrics ベースの計算に統一 | 現在 `_estimate_text_width_pt` は HarfBuzz shaping advance + 末尾 rsb 補正で動くが、`fontTools 4.63.0` / `uharfbuzz 0.54.1` を pip 依存に明示する必要がある (現状 `pip install --break-system-packages` 手動)。`requirements.txt` または `pyproject.toml` に追加し、CI でも fontTools/uharfbuzz が available か確認するテストを追加。fallback path (font 解決不能時に CJK 1.0× / Latin 0.55× の経験係数を使う) は維持しつつ、本番経路では metrics ベースを使うよう default 切替。FONT-001 と組合せると viewer 描画と estimator の差は -2.62pt まで縮小済 (= pixel 量子化レベル)。memory: `reference-text-width-calculation` |
| FONT-004 | todo | P2 | font name 正規化対象 deck の一斉適用 + 再 review snapshot | mcp-cource / 260329 / fix-demo / overflow-fixtures 等の本番 deck に対し FONT-001 を適用、再 fix + render + SPA snapshot publish (rev 番号は各 deck の次の番号)。FONT-002 の wrapper 経由で運用統一できれば本タスクは自動化される。残課題: normalize で typeface が変わる影響を SPA judgement で確認 (1 deck = 1 rev) |
