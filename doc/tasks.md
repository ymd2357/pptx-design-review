...
| FIX-008 | todo | P1 | `card_grid_consistency` の auto-fix を「保守的平行移動のみ」から **child の role / 関係性** を見る賢い修正に進化させる | 現状 (REV-019 で採用された v3) は children の bbox を完全保持し container 内で平行移動するだけで、**視覚的変化が極めて小さく** カード grid が揃って見える効果が出ない。v1 では children width を一律 `target_child_width` に拡大したため image が壊れて全件 reject された。<br>**本質的な原因 (REV-019 v1 失敗の根本)**: 1 つの card container 内に「icon + title text + body text」のような **役割の異なる children** が同居しているのに、全 children を同質に扱って width を一律拡大した。icon (= leading visual marker)、title (= heading)、body (= 本文) は互いに紐付いた配置関係を持っており、`shape kind` (image / text) だけでなく **card 内 role / 相互関係** を見て修正候補を分岐させる必要がある。<br>**設計案 (auto-fix 側)**:<br>(1) text shape のうち本文系は width を `target_child_width` に揃え (= テンプレ意図);<br>(2) image / picture (icon 含む) は **縦横比を維持** し、widening 禁止。icon は元サイズ + グループ相対位置を保つ;<br>(3) child の役割推定: shape の bbox / 順序 / shape_name から `kind` + 加えて `role` (leading_icon / title / body / decorative) を推定;<br>(4) 同 role の children が card 間で揃うようにのみ調整;<br>(5) 安全弁 (container 単位 manual_required skip、v3 実装済) を踏襲。<br>**lint 検知ロジックへの影響**: 現 `card_grid_consistency` は container レベルの bbox / padding median しか見ていない。child の role / 相互関係を見るなら、**「同 role の children が card 間で揃っているか」**を別軸で検知する拡張が必要になる可能性。例: icon の center_x が card 間で揃っているか、title text の baseline が揃っているか、body text の左端 padding が揃っているか。card_grid_consistency lint そのものを拡張するか、別 lint (`card_role_alignment` 等) として切り出すかは設計判断。<br>**検証**: REV-019 と同じ 4 finding (slide 2, 7, 13, 20) を再走 → image 破綻なし + 「カード内の同 role 要素 (icon 列 / title 列 / body 列) が card 間で揃って見える」効果が compare UI で確認できる状態。 |
