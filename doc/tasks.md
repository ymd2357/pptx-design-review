...
| REV-019 | todo | P1 | P2-12 `card_grid_consistency` の修正候補をスライド単位に採否確定する | (1) 入力 `p0-rev-017-low-contrast-fixed.pptx` に対し `pptx_lint --no-consolidate` を当てて `card_grid_consistency` finding 4 件を抽出、(2) 修正候補 (= shape 位置 / 内余白の揃え) が `pptx_fix` で auto 適用可能か確認。auto-fix できない場合は `manual_layout_fix` 系として finding 単位の判定ワークフロー (= 旧視覚レビュー UI) で扱う、(3) auto-fix できる場合は before/after PNG を生成して `/compare/?deck=…&rev=019` でスライド単位採否、(4) どちらでも採否結果を `rev-019-{compare,judgements}.tsv` として確定し、(5) REV-016 観点別判定表の `P2-12` を `done` に。 |
