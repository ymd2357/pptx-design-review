...
| DS-COLOR-001 | done | P1 | design-system loader を作り、lint/fix のハードコード色定数を guideline YAML 参照へ移す (旧 ID: 「Next 表の DS-002」だったが Done 表の DS-002 (トークン命名規則固定) と ID 衝突していたため 2026-05-20 に DS-COLOR-001 へ改名) | **完了 2026-05-20**: `doc/slide-guideline-v1.yml` の `rules.color.lint_palette` を唯一の正本に。`pptx_lint.py` の 6 構造 (`ALLOWED_TEXT_COLORS_HEX` / `ALLOWED_FILL_COLORS_HEX` / `TEXT_COLOR_TOKEN_BY_HEX` / `FILL_COLOR_TOKEN_BY_HEX` / `CONTRAST_REPAIR_COLOR_FAMILIES` / `FILL_REPAIR_COLOR_FAMILIES`) の hardcoded Python 辞書・タプル (合計 234 行) を撤去し、import 時に `design_system_loader.load_lint_palette()` で YAML から populate する形に切替。`PPTX_PALETTE_SOURCE` env switch も撤去 (両バージョンが無くなったため意味なし)。`test_design_system_loader.py` を「両 source 一致」から「YAML が正しく load されて pptx_lint モジュール定数と一致する smoke test」に書き換え、14 text + 49 fill 色を読み込めることを assert。POLICY-001 段階2 で fix_policy を YAML 化したのと同じパターン。全 11 テスト green、挙動不変。 |
...
- レビュー対象の元ファイルは
  `/Users/yamadakenichi/workspace/GitHub/vscode-pptx-viewer/samples/260329_seminar_curriculum_proposal.pptx`。
- 直近の正本候補は
...
| ID | 状態 | 優先度 | タスク | 完了条件 |
| ---- | ------ | -------- | -------- | ---------- |
| FONT-001 | done | P0 | pptx の font 名を OOXML family-only に正規化する前処理を新設 | `pptx_normalize_fonts.py` を追加し、`Calibri (MS) Bold` 等 PowerPoint Mac 由来の PostScript / full name を family-only (`Calibri`) に正規化する。Chromium が CSS `font-family` を解決できず sans-serif fallback で描画していた問題 (= mcp-cource shape 12 で text width +4-9pt/字 ズレ) を根本解消。実測: mcp-cource shape 12 の text width が normalize 前 706.37pt → normalize 後 **699.28pt** (HarfBuzz advance - rsb 推定 701.90pt と差 -2.62pt、pixel 量子化レベル)。weight 情報は `<a:rPr b>` 等で既に保存されているため typeface 側から strip しても描画不変。前段検証は `doc/research/text-width-residual/01-04*.md`。memory: `feedback-pptx-font-normalize`、`reference-text-width-calculation` |
| FONT-002 | done | P0 | raw deck の design lint と測定用 font 正規化を分離して pipeline に組み込む | `pptx_two_layer_lint.py` を追加。raw PPTX で design lint を実行し、normalize 済み派生 deck を生成して測定系 lint を実行する。combined artifact は source path / normalization report / raw findings / normalized findings / review findings を保持し、`font_family` など design findings は raw、`text_box_overflow` / `text_canvas_overflow` / `wrap_break_changes_meaning` / `text_vertical_balance` は normalized measurement layer を採用する。`--verify-fix` で normalized deck に `review_findings` ベースの fixer を適用し、fix actions と after lint も同じ artifact に残す。 |
| FONT-003 | done | P1 | text width estimator に fontTools / uharfbuzz を依存追加し font metrics ベースの計算に統一 | `requirements.txt` に依存を明示。fast 層は HarfBuzz + fontTools + RSB 補正 + OOXML spacing を使う。oracle 層は `text_width_oracle.js` に分離し、検証記録を `doc/research/text-width-residual/05-font-normalize-wrap-fallback-adversarial.md` に残す。 |
| FONT-004 | doing | P2 | font name 正規化対象 deck の一斉適用 + 再 review snapshot | 2026-05-27 初回 two-layer 実行済: `mcp-cource/font-pipeline-rev-007`、`overflow-fixtures/font-pipeline-rev-008`、`260329-seminar-curriculum-proposal/font-pipeline-rev-034`。`publish_review_snapshot.py --lint-json --before-images --after-images` で two-layer `review-artifact.json` と render before/after から compare 用 snapshot を生成可能にした。`260329` は `REV-034` で snapshot / judgement TSV まで完了。`overflow-fixtures` は badge-like 誤判定を修正後、`font-pipeline-rev-009` で `badge_alignment=0` / apply action 0 を確認したため snapshot 対象外。`mcp-cource` は alignment を judgement_fix 化し、card_grid auto 昇格を compact 3-child card に限定した `font-pipeline-policy-retest` で alignment apply 0、card_grid apply 11、fixed lint 2531 件 / error 803 件まで改善。**REV-007 視覚レビュー結果 (2026-06-01)**: changed slides 26 件中 adopt 9 / reject 17。reject 17 件の内訳 = (a) card-like shape への fill_color / contrast bg-mode が card 背景を消去 (slide 9/10/19)、(b) card_grid auto promote が architecture / step / multi-col list を誤認して text を card 外へ移動 (slide 29/31/35/37/39/41/43/46)、(c) geometry の card 内 child 縦移動 (slide 44)、(d) lint 検出漏れ (slide 12/24/28、別軸)。policy 反映: `slide-guideline-v1.yml` の `card_grid_consistency.auto_fix_promote_scope.disabled=true`、`geometry_rounding.auto_fix_promote_scope` に `exclude_group_children / exclude_card_grid_children` を追加、`low_contrast` / `contrast_ratio` に `bg_mode_target_constraint.forbid_card_like_shape=true` を追加、`background_color_palette` に `forbid_card_like_shape=true` を追加 (FONT-005 で実装側に反映)。 |
| FONT-005 | done | P0 | REV-007 evidence ベースの policy 制約を実装側 (`pptx_lint.py` の auto_fix_candidate 昇格判定) に反映する | **lint 側**: `_judgement_auto_fixable` の card_grid_consistency 分岐を return False に固定 (compact 3-child / child_visual_overflow / title_alignment / center_anchor すべて auto promote 不可)。`_geometry_auto_fixable` に `is_group_child` / `is_card_grid_child` 除外を追加し、`check_geometry_rounding` で `_shape_is_group_child` 判定と slide 内 card-container child 集合参照を行うよう拡張。<br>**fix 側**: `_find_bg_repair_target` (`pptx_fix.py`) で enclosing solid fill shape が **2 つ以上の子要素を完全内包** する場合 (= card-like) は target に選ばないガードを追加。slide 9/10/19 で観測した card 背景塗りつぶしを防止。単一 textbox + rect のみの bg-mode は影響を受けない (`_build_behind_solid_fill_contrast_fixture` で確認)。<br>**テスト**: `_card_grid_auto_fixable` predicate 自体は compact-3-child を真と判定する挙動を保持し (テスト互換)、`_judgement_auto_fixable` 経由の昇格のみ無効化したことを `test_pptx_lint.py` に追加 assert。`test_pptx_fix.py` の card_grid 系 4 ブロック (child_visual_overflow / title_alignment / inline_icon default / inline_icon text-center) を `judgement_gate=False` 経由 + fixability=`manual_required` expectation に書き換え、apply 挙動の回帰は維持。全 11 test green。 |
| FONT-006 | todo | P1 | REV-007 で発覚した lint 検出漏れ (slide 12/24/28) を分析・改善する | slide 12 = カード内 text のはみ出しが `text_canvas_overflow` で拾えていない、slide 24 = ブラウザ内テキストの改行修正が `text_box_overflow` で出ていない、slide 28 = 句点レスキューが `wrap_break_changes_meaning` で発火していない。各 finding を再現する fixture を `tmp/review/mcp-cource/font-pipeline-policy-retest/normalized-measurement-lint.json` から特定し、原因を `text_box_overflow` / `text_canvas_overflow` の閾値 or `wrap_break_changes_meaning` の widen_to_fit 条件のどこにあるか切り分ける。 |
| WEB-001 | done | P1 | レビュー UI を GitHub Pages 公開し、KV 経由でローカルに判定を取り込む | 2026-05-19 に fix-demo deck rev-001 で end-to-end 経路が初めて 1 周通過。3 スライド全 adopt で `doc/reviews/fix-demo/rev-001-compare.tsv` に書き戻されたため done に変更。(1) `ymd2357/pptx-design-review` を public で push 済、(2) `deploy-review-web` ワークフローが `review-pages` ブランチに orphan push、(3) Settings → Pages の Source は `review-pages` ブランチ、(4) 画面ゲートは PIN (SHA-256 を SPA に埋め込み、平文は Claude メモリのみ) で通る、(5) Submit ボタンで判定 payload を age 公開鍵で暗号化し `https://pptx-visual-review.pages.dev/api/feedback` に POST、(6) `scripts/fetch-reviews.py --apply` を PC で実行し KV から復号して `doc/reviews/<deck>/rev-NNN-decisions.tsv` & `rev-NNN-finding-judgements.json` を書き出す、(7) `git commit` して反映。**前提**: vscode-pptx-viewer 側の `gallery/functions/api/feedback.js` に `?key=<id>` GET を追加するパッチが production deploy 済 (Claude が当て済、ユーザーが main マージ + push)。**残課題**: fetch-reviews が KV を毎回再走査するため、既存 TSV の memo 列を手書き編集していた場合に上書きされる (今回 `rev-019-compare.tsv` のメモが一時的に消えた)。memo 保持の idempotent 化は別タスク。 |
...
| REV-034 | done | P2 | FONT-004 two-layer normalized deck の fixer 差分を視覚レビュー | 完了。`tmp/review-snapshot/.../rev-034/` に normalized before / fixed after / diff / two-layer `review-artifact.json` を publish。changed slides は 2 / 13 のみ (`AE=22987`, `8056`) で、`rev-034-compare.tsv` では両方 **adopt**。raw design lint 214 件の内訳は `geometry_rounding=159`, `text_box_overflow=31`, `font_family=20`, `card_grid_consistency=4`。 |
...
| FIX-002 | done | P1 | badge コンテナ内テキストの中央揃えを検出・修正する | **lint 側**: 新規 check `badge_alignment` (warning) を追加。`_is_badge_like_shape` で「AUTO_SHAPE + canvas 内 + solid fill ある + 単一行 ≤20 文字 + bbox 240×120pt 以下 + アスペクト比 0.5〜2.5」を満たす shape を badge と分類し、paragraph alignment が CENTER 以外 / vertical_anchor が MIDDLE 以外なら misaligned 軸を列挙して finding 出力。`fixability=auto_fix_candidate` + `candidate_values={alignment:"CENTER", vertical_anchor:"MIDDLE"}`。`check_alignment` は badge shape を skip して二重発火を防止。<br>**fix 側**: `badge_alignment` rule を `FINDING_DRIVEN_RULES` に追加し、`_detect_finding_action` で apply action を生成、`_apply_finding_action` で `text_frame.vertical_anchor = MIDDLE` + 各段落 `alignment = CENTER` を設定。<br>**回帰**: `_make_badge_alignment_bad` (ROUNDED_RECTANGLE 120×48 + 紫 fill + "Beta" + LEFT/TOP) と `_make_badge_alignment_good` (同 shape + CENTER/MIDDLE) を `test_pptx_lint.py` に、`_build_badge_alignment_fixture` を `test_pptx_fix.py` に追加。bad fixture では `misaligned={alignment, vertical_anchor}` + fix 後の residual 0 を assert。REV-017 deck では現在 badge_alignment が 0 件 (false positive 無し)。検証 deck の根拠は `claude-manual-visual-fixes-2026-05-10.md` の slide 3 / 40 |
| LINT-008 | done | P2 | 孤立装飾 line / connector / arrow の検出を追加する | (旧 ID: FIX-003 → 検出 only タスクとして LINT-008 にリネーム、その後 FIX-013 で judgement 経路に拡張) `decorative_isolated_lines` check を新設 (warning)。`MSO_SHAPE_TYPE.LINE` (connector) / `auto_shape_type` 名に `ARROW` または `LINE` を含む autoshape / 極細矩形 (thickness ≤ 4pt + length ≥ 20pt) を line-like と分類し、最近傍 shape の bbox gap が 24pt 以上なら finding を出力。検証 deck の根拠は `claude-manual-visual-fixes-2026-05-10.md` の slide 12 / 44。**FIX-013 移行後 (2026-05-20)**: lint は `fixability=manual_required` + `candidate_values=[{strategy:"remove_shape", shape_id, bbox_pt, ...}]` を出力。POLICY-001 段階3 の strict gate により、SPA で `judgement_reason=auto_fixable` に promote されない限り apply されない。 |
| FIX-013 | done | P2 | LINT-008 (decorative_isolated_lines) に judgement-driven の auto-fix 経路を追加する | YAML `rules.lint.fix_policy.decorative_isolated_lines` を `no_fix` → `judgement_fix` (fix_rule: `decorative_remove`) に変更。`pptx_lint` は `fixability=manual_required` + `candidate_values=[{strategy:"remove_shape"}]` を出力 (旧 `fixability=decorative_review` ハードコードと `_fixability_for_json` の特別扱いは撤去)。`pptx_fix` に新規 rule `decorative_remove` を追加し ALL_RULES / FINDING_DRIVEN_RULES に登録、`_detect_finding_action` と `_apply_finding_action` に分岐 (`shape._element.getparent().remove(shape._element)` で slide spTree から物理削除)。POLICY-001 段階3 の strict gate と整合: 既定では SPA judgement (auto_fixable promote) が無いと apply しない。`test_pptx_fix.py` に `_build_decorative_isolated_lines_fixture` と end-to-end 回帰 (gate=True で shape 数不変、judgement promote 後に 1 shape 削除) を追加。`test_fix_policy.py` は自動的に新分類 (judgement_fix=24 / no_fix=5) を picksup。 |
...
| POLICY-001 | done | P1 | lint→fix の policy を YAML 正本テーブルに統一する | **段階1 (完了)**: `doc/slide-guideline-v1.yml` に `rules.lint.fix_policy` セクションを新設し、36 check 分の `{apply_mode: auto_fix/judgement_fix/no_fix, fix_rule: <rule>/null}` を宣言的に列挙。`apply_mode=auto_fix` (5: text_autofit / font_size / line_height / badge_alignment / box_canvas_overflow)、`judgement_fix` (26: contrast / bbox_fit / overlap / spacing / image_aspect / image_crop / reading_order / animation / text_wrap / heading_hierarchy / inner_padding / card_grid / text_vertical_balance / font_family / alignment / text_color / fill_color / geometry / object_gap_too_small / decorative_isolated_lines 含む)、`no_fix` (5: image_upscale_ratio / alt_text_required / color_only_meaning / missing_required_element / slide_size)。`skills/pptx-design-reviewer/scripts/test_fix_policy.py` で YAML 正本と `pptx_fix.CHECK_TO_RULE` の sync を assert。**段階2 (完了)**: `pptx_fix.CHECK_TO_RULE` を `_load_check_to_rule()` 経由で YAML 由来に refactor (PyYAML 不在時のテキストパーサ fallback 付き)。`pptx_lint.py:_fixability_for_json` を `FIX_POLICY` 参照に書き換え、`apply_mode` で no_fix/auto_fix/judgement_fix に分岐 → `_judgement_auto_fixable` (条件付昇格: geometry / wrap_break / contrast / card_grid) + `_judgement_manual_reason` で context-aware reason を返す構造に整理。reason 文字列だけは `_AUTO_FIX_REASONS` / `_JUDGEMENT_AUTO_FIX_REASONS` / `MANUAL_REQUIRED_REASONS` 辞書に残置 (i18n 改修は別)。全 9 既存テスト + test_fix_policy が green、挙動不変を担保。**命名 (2026-05-20)**: 「manual」は紛らわしい (手動 fix 経路があるとも読める) というユーザー指摘で 3 値とも `_fix` で揃える: auto → `auto_fix`、judgement → `judgement_fix`、manual → `no_fix`。**段階3 (完了 2026-05-20)**: `pptx_fix.fix_pptx(..., judgement_gate=True)` をデフォルトに。`apply_mode=judgement_fix` の check で `fixability=manual_required` の finding は SPA judgement (= `auto_fix_candidate` promote) が無い限り apply されず `judgement_fix_gate_requires_spa_judgement` reason で skip。条件付昇格 (geometry / wrap_break / contrast / card_grid) で lint が auto_fix_candidate を出した場合はそのまま apply (= 既存挙動互換)。CLI に `--no-judgement-gate` を追加し、REV-017/019 等の既存パイプラインは flag 経由で従来挙動を維持可能。新規 `test_judgement_gate.py` で gate=True/False 両挙動と `_apply_mode_for_check` lookup を verify。既存 `test_pptx_fix.py` / `test_pptx_fix_evidence_schema.py` の judgement_fix-policy fixture には `judgement_gate=False` を明示。 |
