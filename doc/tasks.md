...
| FONT-002 | todo | P0 | raw deck の design lint と測定用 font 正規化を分離して pipeline に組み込む | `font_family` は design-system typography 由来の embedded typeface 完全一致で判定する。raw PPTX で design lint を実行し、text width 推定・viewer render・fix 検証だけ normalize 済み派生 deck を使う。 |
