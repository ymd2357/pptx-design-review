...
| FIX-010 | todo | P2 | contrast bg-mode の `behind_solid_fill` 経路を実画像 fixture で検証 | FIX-006 で `_find_bg_repair_target` が text shape の外側の enclosing fill shape を探し当てて bg-mode auto-fix を適用するロジックを実装したが、自動テストは `shape_solid_fill` (text shape の自前 fill) 経路のみカバー。`behind_solid_fill:<label>` 経路 (= text shape の背後にある rect の fill を差し替え) は実 PPTX で踏まないと回帰検知できない。<br>**やること**: (a) `tmp/review/.../fixtures/` 配下に「rect (fill=#FF5757) の上に textbox (fg=#FFFFFF, 元色維持) を重ねた」テスト PPTX を作る、(b) `pptx_lint` が `background_source=behind_solid_fill:...` で contrast finding を出すこと、(c) `pptx_fix --rules contrast` を流すと **textbox は変わらず enclosing rect の fill だけが変わる** ことを XML レベルで確認、(d) test_pptx_fix.py に該当 fixture 検証を追加。 |
| FIX-012 | todo | P1 | `card_grid_consistency` の auto-fix を role 推定 + wrap 事前検査 + vertical alignment 切替で再設計 | FIX-008 v4 (slot-aware) が REV-032 で全件 reject された反省を踏まえる。<br>**前提**: 現状の master は v3 (保守的平行移動のみ)。`pptx_lint` の `row_containers` は児童データを持たず、`inconsistent_containers` 側に `children` がある。<br>**新設計の柱**:<br>(1) **役割 (role) 推定**: 各 child を `leading_icon` / `title` / `body` / `decorative` のいずれかに分類する。判定材料は shape kind (`image` / `text` / `shape`) + bbox の高さ・幅・top 順序 + (将来) 文字数 / font_size。レビューワーの言葉では「アイコン」「ヘッダー」「本文」の 3 系統。<br>(2) **カード間の role 整列**: 同 role の children を **カード間で実際に揃える**。`leading_icon` は center_x を行内 median に、`title` は baseline (= top + ascent) を行内 median に、`body` は左端 padding を行内 median に。slot index による各カード独立の平行移動では不十分 (= v4 の失敗 (a))。<br>(3) **text width 変更前の wrap 事前検査**: text shape の width を変えるときは、変更後の幅で `python-pptx` 等で **行数が増えないか** を事前計算 (autosize 設定 / 既存テキスト長 / フォント幅近似)。行数が増える見込みなら適用 skip → `manual_required`。v4 では slide 13 / 20 で「不要な改行」が出た (= 失敗 (b))。<br>(4) **行数差に応じた vertical alignment**: 同 row のカード間で text 行数が混在するとき (1 行 / 2 行 / 3 行)、vertical anchor を **行数差に応じて切り替える** (例: 全カード body が 2 行なら anchor は元のまま; 1 行が混じるなら他カード anchor を `TOP`; 3 行が混じるなら他カード anchor を `BOTTOM`)。v4 では vertical anchor を扱っていなかった (= 失敗 (c))。<br>(5) **lint 側拡張**: `card_grid_consistency` の triggered_rules に「role 別整列ズレ」を追加するか、別 lint `card_role_alignment` として切り出すかを設計判断。`pptx_lint` 側で各 child の role を推定し row_containers に埋めれば、`pptx_fix` の役割整列ロジックと共有できる。<br>**検証計画**: REV-019 / REV-032 と同じ deck で再走し (a) image 破綻なし (cx/cy 不変) (b) 同 role の整列が render 上見えて確認できる (c) wrap 起因の不要な改行ゼロ (d) 行数差が混在するカード行で vertical alignment が適切に切り替わる、を compare UI で全採用される状態。 |
