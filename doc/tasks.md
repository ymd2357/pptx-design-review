...
| FONT-005 | todo | P0 | REV-007 evidence ベースの policy 制約を実装側 (`pptx_lint.py` の auto_fix_candidate 昇格判定) に反映する | YAML の `fix_policy.<check>.auto_fix_promote_scope` / `bg_mode_target_constraint` を `_judgement_auto_fixable` および contrast `_find_bg_repair_target` の制約として実装する。具体的には: (1) `card_grid_consistency` の auto promote 全廃 (compact 3-child 例外も無効化)、(2) `geometry_rounding` 昇格時に shape が group child / card_grid child でないことを check、(3) contrast bg-mode の target に「card-like (= 内部に子テキスト/icon を含む) shape」を選ばない (子要素検査を追加)、(4) `background_color_palette` の auto promote (もしあれば) で card-like shape を除外。回帰テストとして REV-007 deck で再 lint → auto_fix_candidate が `card_grid` 0 件、`geometry_rounding` で card 内部 shape 0 件、`contrast/fill_color` で card-like target 0 件になることを assert。 |
